...
$PreviousResults = 'D:\agc_work\stage65_results'
$OutputDir       = 'D:\agc_work\stage66_results'

$Sdk = '/opt/ps5-payload-sdk'

$SprxWsl     = '/mnt/d/agc_work/sce_stubs/libSceAgcDriver.sprx'
$NidDbWsl    = '/mnt/d/sdk-master/sce_stubs/aerolib.csv'
$PreviousWsl = '/mnt/d/agc_work/stage65_results'
$OutputWsl   = '/mnt/d/agc_work/stage66_results'
$WorkWsl     = '/tmp/agc_stage66'

$AnalyzerWindows = Join-Path $OutputDir 'analyze_dispatch_bounds.py'
$AnalyzerWsl     = "$OutputWsl/analyze_dispatch_bounds.py"

$ReportPath = Join-Path $OutputDir 'STAGE66_REPORT.json'
...
Write-Host 'AGC PS5 Stage 66 - Dispatch Bounds / Dynamic Initialization Audit' -ForegroundColor Cyan
Write-Host '============================================' -ForegroundColor Cyan
...
if (-not (Test-Path -LiteralPath $PreviousResults -PathType Container)) {
    throw "No existe Stage 65 results: $PreviousResults"
}
...
cp $(Quote-Bash $AnalyzerWsl) \
   $(Quote-Bash "$WorkWsl/analyze_dispatch_bounds.py")
...
Write-Host ''
Write-Host '==> Corregir límites de tabla y rastrear inicialización dinámica' -ForegroundColor Yellow

Invoke-Wsl @"
set -e

python3 $(Quote-Bash "$WorkWsl/analyze_dispatch_bounds.py") \
    $(Quote-Bash $SprxWsl) \
    $(Quote-Bash $NidDbWsl) \
...
Write-Host ''
Write-Host '==> Verificar artefactos Stage 66' -ForegroundColor Yellow

Invoke-Wsl @"
set -e

test -f $(Quote-Bash "$OutputWsl/stage66_static.json")
test -f $(Quote-Bash "$OutputWsl/dispatch_bounds_summary.txt")
test -f $(Quote-Bash "$OutputWsl/dispatch_bounds_disassembly.txt")
test -f $(Quote-Bash "$OutputWsl/dispatch_bounds.json")

echo '--- dispatch_bounds_summary.txt ---'

cat $(Quote-Bash "$OutputWsl/dispatch_bounds_summary.txt")
...
$GlobalRw =
    [bool]$Static.conclusions.GLOBAL_RW_SEGMENT_IDENTIFIED

$SubmitBoundary =
    [bool]$Static.conclusions.SUBMIT_STATIC_SLOT_BOUNDARY_PROVEN

$MultiBoundary =
    [bool]$Static.conclusions.MULTI_STATIC_SLOT_BOUNDARY_PROVEN

$Stage65Correction =
    [bool]$Static.conclusions.STAGE65_EXTERNAL_REGION_READS_IDENTIFIED

$SubmitDynamic =
    [bool]$Static.conclusions.SUBMIT_DYNAMIC_TABLE_STORE_CANDIDATE_FOUND

$MultiDynamic =
    [bool]$Static.conclusions.MULTI_DYNAMIC_TABLE_STORE_CANDIDATE_FOUND

$RuntimeInitCandidate =
    [bool]$Static.conclusions.DISPATCH_TABLE_RUNTIME_INITIALIZATION_CANDIDATE_FOUND
...
Write-Host "[INFO] stage66_static.json SHA256=$HashStatic"
Write-Host "[INFO] dispatch_bounds_summary.txt SHA256=$HashSummary"
Write-Host "[INFO] dispatch_bounds_disassembly.txt SHA256=$HashDisassembly"
Write-Host "[INFO] dispatch_bounds.json SHA256=$HashFocused"
...
$Report = [ordered]@{
    ...
    conclusions = [ordered]@{
        GLOBAL_RW_SEGMENT_IDENTIFIED = $GlobalRw
        SUBMIT_STATIC_SLOT_BOUNDARY_PROVEN = $SubmitBoundary
        MULTI_STATIC_SLOT_BOUNDARY_PROVEN = $MultiBoundary

        STAGE65_EXTERNAL_REGION_READS_IDENTIFIED = $Stage65Correction

        SUBMIT_DYNAMIC_TABLE_STORE_CANDIDATE_FOUND = $SubmitDynamic
        MULTI_DYNAMIC_TABLE_STORE_CANDIDATE_FOUND = $MultiDynamic
        DISPATCH_TABLE_RUNTIME_INITIALIZATION_CANDIDATE_FOUND = $RuntimeInitCandidate
    ...
    hashes = [ordered]@{
        'stage66_static.json' = $HashStatic
        'dispatch_bounds_summary.txt' = $HashSummary
        'dispatch_bounds_disassembly.txt' = $HashDisassembly
        'dispatch_bounds.json' = $HashFocused
    }
}
...
Write-Host 'Stage 66 completado' -ForegroundColor Green
Write-Host '============================================' -ForegroundColor Green
Write-Host ''

Write-Host "GLOBAL_RW_SEGMENT_IDENTIFIED = $GlobalRw"
Write-Host "SUBMIT_STATIC_SLOT_BOUNDARY_PROVEN = $SubmitBoundary"
Write-Host "MULTI_STATIC_SLOT_BOUNDARY_PROVEN = $MultiBoundary"

Write-Host "STAGE65_EXTERNAL_REGION_READS_IDENTIFIED = $Stage65Correction"

Write-Host "SUBMIT_DYNAMIC_TABLE_STORE_CANDIDATE_FOUND = $SubmitDynamic"
Write-Host "MULTI_DYNAMIC_TABLE_STORE_CANDIDATE_FOUND = $MultiDynamic"
Write-Host "DISPATCH_TABLE_RUNTIME_INITIALIZATION_CANDIDATE_FOUND = $RuntimeInitCandidate"
