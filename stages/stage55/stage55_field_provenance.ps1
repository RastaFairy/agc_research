...
param(
    ...
    [string]$PreviousResults = 'D:\agc_work\stage54_results',
    [string]$OutDir = 'D:\agc_work\stage55_results',
...
$WorkWsl = '/tmp/agc_stage55'

$AnalyzerWin = Join-Path `
    $OutDir `
    'analyze_field_provenance.py'

$AnalyzerWsl = "$OutWsl/analyze_field_provenance.py"
...
cp $(Quote-Bash $AnalyzerWsl) \
   $(Quote-Bash "$WorkWsl/analyze_field_provenance.py")
...
Write-Step `
    'Analizar procedencia exacta de los campos'

$AnalyzeCommand = @"
set -e

python3 $(Quote-Bash "$WorkWsl/analyze_field_provenance.py") \
    $(Quote-Bash $SprxWsl) \
    $(Quote-Bash $NidDbWsl) \
...
Write-Step `
    'Verificar artefactos Stage 55'

$VerifyArtifactsCommand = @"
set -e

test -f $(Quote-Bash "$OutWsl/stage55_static.json")
test -f $(Quote-Bash "$OutWsl/field_provenance_summary.txt")
test -f $(Quote-Bash "$OutWsl/submit_family_disassembly.txt")

echo '--- field_provenance_summary.txt ---'
cat $(Quote-Bash "$OutWsl/field_provenance_summary.txt")
...
$CanonicalRecord = [bool](
    $Static.conclusions.CANONICAL_0x20_RECORD_LAYOUT_PROVEN
)

$Field00Proven = [bool](
    $Static.conclusions.FIELD_00_PROVEN_AS_64BIT_ARRAY_VALUE
)

$Field08Proven = [bool](
    $Static.conclusions.FIELD_08_PROVEN_AS_32BIT_ARRAY_VALUE
)

$Field0CZero = [bool](
    $Static.conclusions.FIELD_0C_PROVEN_ZERO_INITIALIZED_BY_MULTI
)

$StrideProven = [bool](
    $Static.conclusions.ARGUMENT_RECORD_STRIDE_PROVEN
)
...
Write-Host `
    "[INFO] stage55_static.json SHA256=$HashStatic"

Write-Host `
    "[INFO] field_provenance_summary.txt SHA256=$HashSummary"
...
Write-Host 'Stage 55 completado' -ForegroundColor Green
Write-Host '============================================' -ForegroundColor Green
Write-Host ''

Write-Host `
    "CANONICAL_0x20_RECORD_LAYOUT_PROVEN = $CanonicalRecord"

Write-Host `
    "FIELD_00_PROVEN_AS_64BIT_ARRAY_VALUE = $Field00Proven"

Write-Host `
    "FIELD_08_PROVEN_AS_32BIT_ARRAY_VALUE = $Field08Proven"

Write-Host `
    "FIELD_0C_PROVEN_ZERO_INITIALIZED_BY_MULTI = $Field0CZero"

Write-Host `
    "ARGUMENT_RECORD_STRIDE_PROVEN = $StrideProven"
