...
# ============================================================
# AGC PS5 Stage 69
# Dispatch Table Count / +0xA0 Semantic Audit
# ============================================================
...
$PreviousStageDir = 'D:\agc_work\stage68_results'
$OutputDir = 'D:\agc_work\stage69_results'

$SprxWsl = '/mnt/d/agc_work/sce_stubs/libSceAgcDriver.sprx'
$PreviousWsl = '/mnt/d/agc_work/stage68_results'
$OutputWsl = '/mnt/d/agc_work/stage69_results'

$StageTmpWsl = '/tmp/agc_stage69'
...
Write-Host ''
Write-Host '============================================'
Write-Host 'AGC PS5 Stage 69 - Dispatch Table Count / +0xA0 Semantic Audit'
Write-Host '============================================'
...
if (-not (Test-Path -LiteralPath $PreviousStageDir)) {
    throw "No existe el directorio de la etapa previa: $PreviousStageDir"
}
...
function To-WslPath {
    ...
    throw "No se pudo convertir a ruta WSL: $WindowsPath"
}
...
$AnalyzerPath = Join-Path $OutputDir 'analyze_dispatch_count.py'
...
# UTF-8 SIN BOM para que Python y WSL lo reciban limpiamente.
...
$AnalyzerWsl = To-WslPath -WindowsPath $AnalyzerPath

Write-Host ''
Write-Host '==> Validar analizador Stage 69'

$AnalyzerPrepareCommand = @"
cp '$AnalyzerWsl' '$StageTmpWsl/analyze_dispatch_count.py'
sed -i 's/\r$//' '$StageTmpWsl/analyze_dispatch_count.py'
python3 -m py_compile '$StageTmpWsl/analyze_dispatch_count.py'
ls -lh '$StageTmpWsl/analyze_dispatch_count.py'
"@

Invoke-WslStep `
    -Label 'Validar analizador Stage 69' `
    -Command $AnalyzerPrepareCommand | Out-Null
...
Write-Host ''
Write-Host '==> Ejecutar auditoría de semántica +0xA0'

$RunCommand = @"
python3 '$StageTmpWsl/analyze_dispatch_count.py' \
    '$SprxWsl' \
...
Invoke-WslStep `
    -Label 'Ejecutar auditoría de semántica +0xA0' `
...
Write-Host ''
Write-Host '==> Verificar artefactos Stage 69'

$VerifyCommand = @"
set -e

test -f '$OutputWsl/stage69_static.json'
test -f '$OutputWsl/dispatch_count_summary.txt'
test -f '$OutputWsl/dispatch_count_disassembly.txt'
test -f '$OutputWsl/dispatch_count_correlation.json'
test -f '$OutputWsl/STAGE69_REPORT.json'

echo '--- dispatch_count_summary.txt ---'
cat '$OutputWsl/dispatch_count_summary.txt'
...
Invoke-WslStep `
    -Label 'Verificar artefactos Stage 69' `
...
foreach ($FileName in $HashFiles) {
    $FullPath = Join-Path $OutputDir $FileName

    if (Test-Path -LiteralPath $FullPath) {
        $Hash = Get-Sha256 -Path $FullPath
        Write-Host "[INFO] $FileName SHA256=$Hash"
    }
}

# ------------------------------------------------------------
# Limpieza final del workspace temporal
# ------------------------------------------------------------

$CleanupCommand = @"
rm -f '$StageTmpWsl/.stage69_tmp.bin'
"@

Invoke-WslStep `
    -Label 'Limpiar temporales Stage 69' `
    -Command $CleanupCommand | Out-Null

# IMPORTANTE:
# NO hacemos Copy-Item del analyzer porque ya se ha creado directamente
# en D:\agc_work\stage69_results. Este era el fallo del script anterior.

$ReportJsonPath = Join-Path $OutputDir 'STAGE69_REPORT.json'

if (-not (Test-Path -LiteralPath $ReportJsonPath)) {
    throw "No se generó STAGE69_REPORT.json"
}

$Report = Get-Content `
    -LiteralPath $ReportJsonPath `
    -Raw |
    ConvertFrom-Json

Write-Host ''
Write-Host '============================================'
Write-Host 'Stage 69 completado'
Write-Host '============================================'
