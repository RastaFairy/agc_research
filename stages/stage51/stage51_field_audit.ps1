...
param(
    ...
    [string]$OutDir = 'D:\agc_work\stage51_results',
...
$WorkWsl = '/tmp/agc_stage51'

$AnalyzerWin = Join-Path `
    $OutDir `
    'analyze_field_types.py'

$AnalyzerWsl = "$OutWsl/analyze_field_types.py"

$StaticWsl = "$WorkWsl/stage51_static.json"
$SummaryWsl = "$WorkWsl/field_types_summary.txt"
$DisasmWsl = "$WorkWsl/submit_commandbuffer_disassembly.txt"
...
Write-Section 'AGC PS5 Stage 51 - Precise Field Type / Access Audit'
...
cp $(Quote-Bash $AnalyzerWsl) \
   $(Quote-Bash "$WorkWsl/analyze_field_types.py")
...
Write-Step 'Analizar tamaño y dirección exacta de los accesos'

$Analyze = @"
set -e

python3 $(Quote-Bash "$WorkWsl/analyze_field_types.py") \
    $(Quote-Bash $SprxWsl) \
    $(Quote-Bash $NidDbWsl) \
    $(Quote-Bash $WorkWsl)
"@
...
Write-Step 'Recopilar artefactos'
...
cp $(Quote-Bash $StaticWsl) \
   $(Quote-Bash "$OutWsl/stage51_static.json")

cp $(Quote-Bash $SummaryWsl) \
   $(Quote-Bash "$OutWsl/field_types_summary.txt")

cp $(Quote-Bash $DisasmWsl) \
   $(Quote-Bash "$OutWsl/submit_commandbuffer_disassembly.txt")

rm -rf $(Quote-Bash "$OutWsl/functions")

cp -r $(Quote-Bash "$WorkWsl/functions") \
      $(Quote-Bash "$OutWsl/functions")

echo '--- field_types_summary.txt ---'
cat $(Quote-Bash $SummaryWsl)
...
$ContextRefs = [int]$Static.context_reference_count
$ContextAccesses = [int]$Static.context_access_count
$PreciseWidths = [int]$Static.precise_width_count
$SubmitFields = [int]$Static.submit_argument_field_count
$ContextWrites = [int]$Static.context_write_count
$ContextRmw = [int]$Static.context_rmw_count

$Tracking = [bool](
    $Static.conclusions.CONTEXT_REGISTER_TRACKING
)

$Widths = [bool](
    $Static.conclusions.CONTEXT_ACCESS_SIZES_IDENTIFIED
)

$Arguments = [bool](
    $Static.conclusions.SUBMIT_ARGUMENT_FIELDS_IDENTIFIED
)

$Writes = [bool](
    $Static.conclusions.CONTEXT_WRITES_FOUND
)

$Rmw = [bool](
    $Static.conclusions.CONTEXT_RMW_FOUND
)
...
Write-Host `
    "[INFO] stage51_static.json SHA256=$HashStatic"

Write-Host `
    "[INFO] field_types_summary.txt SHA256=$HashSummary"

Write-Host `
    "[INFO] submit_commandbuffer_disassembly.txt SHA256=$HashDisasm"
...
$Report = [ordered]@{
    ...
    counts = [ordered]@{
        context_references = $ContextRefs
        context_accesses = $ContextAccesses
        precise_access_widths = $PreciseWidths
        submit_argument_fields = $SubmitFields
        context_writes = $ContextWrites
        context_rmw = $ContextRmw
    }

    conclusions = [ordered]@{
        CONTEXT_REGISTER_TRACKING = $Tracking
        CONTEXT_ACCESS_SIZES_IDENTIFIED = $Widths
        SUBMIT_ARGUMENT_FIELDS_IDENTIFIED = $Arguments
        CONTEXT_WRITES_FOUND = $Writes
        CONTEXT_RMW_FOUND = $Rmw
        ABI_PROTOTYPE_INFERRED = $false
        EXECUTED_AGC = $false
    }

    hashes = [ordered]@{
        'stage51_static.json' = $HashStatic
        'field_types_summary.txt' = $HashSummary
        'submit_commandbuffer_disassembly.txt' = $HashDisasm
    }
}
...
Write-Host 'Stage 51 completado' -ForegroundColor Green
Write-Host '============================================' -ForegroundColor Green
Write-Host ''
