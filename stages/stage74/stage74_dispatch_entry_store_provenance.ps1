...
$PrevWin   = 'D:\agc_work\stage73_results'
$OutputWin = 'D:\agc_work\stage74_results'
...
function Invoke-WslBash {
    param([Parameter(Mandatory)][string]$Script, [string]$Label = 'WSL')
...
Write-Host '============================================'
Write-Host 'AGC PS5 Stage 74 - Dispatch Entry Store / Provenance Audit'
Write-Host '============================================'
...
$AnalyzerPath = Join-Path $OutputWin 'analyze_dispatch_entry_stores.py'
...
Set-Content -Path $AnalyzerPath -Value $PythonAnalyzer -Encoding UTF8

Write-Host "`n==> Preparar workspace Linux"
Invoke-WslBash @"
set -e
rm -rf '/tmp/agc_stage74'
mkdir -p '/tmp/agc_stage74'
mkdir -p '$OutputWsl'
test -f '$SprxWsl'
test -f '$NidDbWsl'
test -d '$PrevWsl'
test -f '$AnalyzerWsl'
cp '$AnalyzerWsl' '/tmp/agc_stage74/analyze_dispatch_entry_stores.py'
sed -i 's/\r$//' '/tmp/agc_stage74/analyze_dispatch_entry_stores.py'
python3 -m py_compile '/tmp/agc_stage74/analyze_dispatch_entry_stores.py'
ls -lh '/tmp/agc_stage74/analyze_dispatch_entry_stores.py'
"@ 'Preparar workspace Linux' | Out-Null
...
Write-Host "`n==> Ejecutar auditoría Stage 74"
Invoke-WslBash @"
set -e
python3 '/tmp/agc_stage74/analyze_dispatch_entry_stores.py' \
    '$SprxWsl' \
    '$NidDbWsl' \
    '$PrevWsl' \
    '$OutputWsl'
"@ 'Stage74 analyzer' | Out-Null

Write-Host "`n==> Verificar artefactos Stage 74"
Invoke-WslBash @"
set -e
for f in \
    stage74_static.json \
    dispatch_entry_provenance_summary.txt \
    dispatch_entry_provenance_disassembly.txt \
    dispatch_entry_provenance.json \
    STAGE74_REPORT.json
 do
    test -f '$OutputWsl/$f'
done
cat '$OutputWsl/dispatch_entry_provenance_summary.txt'
"@ 'Artifact verification' | Out-Null

Write-Host "`n==> Hash artefactos"
Invoke-WslBash "set -e; sha256sum '$OutputWsl/stage74_static.json' '$OutputWsl/dispatch_entry_provenance_summary.txt' '$OutputWsl/dispatch_entry_provenance_disassembly.txt' '$OutputWsl/dispatch_entry_provenance.json' '$OutputWsl/STAGE74_REPORT.json'" 'Hashing' | Out-Null

Write-Host "`n==> Limpiar temporales Stage 74"
Invoke-WslBash "rm -rf '/tmp/agc_stage74'" 'Cleanup' | Out-Null

$ReportJson = Join-Path $OutputWin 'STAGE74_REPORT.json'
if (-not (Test-Path -LiteralPath $ReportJson)) { throw "No se generó $ReportJson" }
$Report = Get-Content -LiteralPath $ReportJson -Raw | ConvertFrom-Json

Write-Host '============================================'
Write-Host 'Stage 74 completado'
Write-Host '============================================'
foreach ($p in @(
    'STORE_SCAN_COMPLETED',
    'STORE_CANDIDATES_FOUND',
    'GLOBAL_CONTEXT_BASE_PROVEN_FOR_ANY_STORE',
    'SUBMIT_TABLE_INITIALIZER_STORE_PROVEN',
    'MULTI_TABLE_INITIALIZER_STORE_PROVEN',
    'ENTRY_0x00_INDEXED_SLOT_PROVEN',
    'ENTRY_0x08_INDEXED_SLOT_PROVEN',
    'ENTRY_STRIDE_PROVEN',
    ...
    $node = $Report.conclusions.PSObject.Properties[$p]
    if ($null -ne $node) {
        Write-Host ("{0} = {1}" -f $p, $node.Value)
    } else {
        Write-Host ("{0} = <no disponible>" -f $p)
    }
}
Write-Host "`nResultados:`n  $OutputWin"
Write-Host "`nReporte:`n  $ReportJson"
