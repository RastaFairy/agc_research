...
param(
    ...
    [string]$Stage82       = $(if ($env:AGC_STAGE82) { $env:AGC_STAGE82 } else { 'D:\agc_work\stage82_results' }),
    [string]$Output        = $(if ($env:AGC_STAGE83) { $env:AGC_STAGE83 } else { 'D:\agc_work\stage83_results' }),
    [string]$KytyRoot      = $(if ($env:KYTYPLUS_ROOT) { $env:KYTYPLUS_ROOT } else { 'D:\agc_sources\KytyPlus' }),
    [string]$KytyZip       = $(if ($env:KYTYPLUS_ZIP) { $env:KYTYPLUS_ZIP } else { 'D:\rpcs3_ps5\KytyPlus-main.zip' }),
...
Write-Section 'AGC PS5 Stage 83 - Semantic Field Proof'
Write-Host "Stage79      = $Stage79"
Write-Host "Stage82      = $Stage82"
Write-Host "Output       = $Output"
Write-Host "KytyRoot     = $KytyRoot"
Write-Host "KytyZip      = $KytyZip"
...
$Stage79 = Resolve-ExistingDirectory $Stage79
$Stage82 = Resolve-ExistingDirectory $Stage82
New-Item -ItemType Directory -Force -Path $Output | Out-Null
$Output = (Resolve-Path -LiteralPath $Output).Path

Require-File (Join-Path $Stage79 'ABI_V1_CONTRACT.txt') 'Stage79 ABI_V1_CONTRACT.txt'
Require-File (Join-Path $Stage82 'submit_18b0_disassembly.txt') 'Stage82 submit_18b0_disassembly.txt'
Require-File (Join-Path $Stage82 'consumer_1000_disassembly.txt') 'Stage82 consumer_1000_disassembly.txt'
Require-File (Join-Path $Stage82 'STAGE82_REPORT.json') 'Stage82 STAGE82_REPORT.json'

Write-Step 'Resolver fuente KytyPlus real'

$KytyMode = $null
$KytyResolved = $null

$KytyCpp = Join-Path $KytyRoot 'src\libs\agc.cpp'
if (Test-Path -LiteralPath $KytyCpp -PathType Leaf) {
    $KytyMode = 'directory'
    $KytyResolved = (Resolve-Path -LiteralPath $KytyRoot).Path
    Write-Host "[OK] KytyPlus checkout -> $KytyCpp"
} elseif (Test-Path -LiteralPath $KytyZip -PathType Leaf) {
    $KytyMode = 'zip'
    $KytyResolved = (Resolve-Path -LiteralPath $KytyZip).Path
    Write-Host "[OK] KytyPlus ZIP -> $KytyResolved"
} else {
    throw "No se encontró KytyPlus/src/libs/agc.cpp ni el ZIP de fuente. Revise KYTYPLUS_ROOT o KYTYPLUS_ZIP."
}

$Stage79Wsl = Convert-ToWslPath $Stage79
$Stage82Wsl = Convert-ToWslPath $Stage82
$OutputWsl  = Convert-ToWslPath $Output
$KytyWsl    = Convert-ToWslPath $KytyResolved
$AnalyzerWin = Join-Path $Output 'stage83_verify.py'
$AnalyzerWsl = Convert-ToWslPath $AnalyzerWin
...
Write-Utf8NoBom -Path $AnalyzerWin -Content $AnalyzerPy

Write-Step 'Validar sintaxis del verificador antes de ejecutarlo'

$Prepare = @"
set -e
mkdir -p $(Quote-Bash $OutputWsl)
sed -i 's/\r$//' $(Quote-Bash $AnalyzerWsl)
python3 -m py_compile $(Quote-Bash $AnalyzerWsl)
test -s $(Quote-Bash $AnalyzerWsl)
echo 'PY_COMPILE=OK'
"@
Invoke-WslBash $Prepare

Write-Step 'Ejecutar Stage 83 sobre artefactos reales'

$Run = @"
set -e
test -f $(Quote-Bash "$Stage79Wsl/ABI_V1_CONTRACT.txt")
test -f $(Quote-Bash "$Stage82Wsl/submit_18b0_disassembly.txt")
test -f $(Quote-Bash "$Stage82Wsl/consumer_1000_disassembly.txt")
test -f $(Quote-Bash "$Stage82Wsl/STAGE82_REPORT.json")
if [ -d $(Quote-Bash $KytyWsl) ]; then
    test -f $(Quote-Bash "$KytyWsl/src/libs/agc.cpp")
else
    test -f $(Quote-Bash $KytyWsl)
fi
python3 $(Quote-Bash $AnalyzerWsl) \
    $(Quote-Bash "$Stage79Wsl/ABI_V1_CONTRACT.txt") \
    $(Quote-Bash "$Stage82Wsl/submit_18b0_disassembly.txt") \
    $(Quote-Bash "$Stage82Wsl/consumer_1000_disassembly.txt") \
    $(Quote-Bash "$Stage82Wsl/STAGE82_REPORT.json") \
    $(Quote-Bash $KytyWsl) \
    $(Quote-Bash "$OutputWsl/STAGE83_REPORT.json")
"@
Invoke-WslBash $Run

Write-Step 'Escribir contrato Stage 83'

$Contract = @'
AGC PS5 STAGE 83 - SEMANTIC FIELD PROOF
=======================================

ENTRADAS CONTRASTADAS
---------------------
- Stage79 ABI_V1_CONTRACT.txt
- Stage82 submit_18b0_disassembly.txt
- Stage82 consumer_1000_disassembly.txt
- Stage82 STAGE82_REPORT.json
- KytyPlus/src/libs/agc.cpp (checkout o ZIP real)

CERRADO
-------
public +0x00 = dcb_gpu_addr
public +0x08 = dcb_num_dwords

DERIVACION
----------
public +0x00 -> submit local -0x40 -> backend +0x10
public +0x08 -> submit local -0x38 -> backend +0x18
public +0x0C -> submit local -0x34 -> backend +0x1C

KytyPlus real contiene Packet.addr, Packet.dw_num y Packet.pad[4].
GraphicsDriverSubmitDcb pasa packet->addr y packet->dw_num a submit_dcb().

NO CERRADO
----------
public +0x0C sigue sin semantica publica Sony definitiva.
La correspondencia con Packet.pad[0] es compatible, pero Stage 83 no la eleva
a una afirmacion definitiva sobre el ABI publico Sony.

NO DEMUESTRA
------------
- firma publica Sony definitiva;
- NID nuevo definitivo;
- ejecucion real en PS5;
- runtime GPU;
- procedencia de la llamada al initializer 0xD0.

STATUS
------
FIELD_00_IS_DCB_GPU_ADDR = true
FIELD_08_IS_DCB_NUM_DWORDS = true
FIELD_0C_PADDING_CONFIRMED = false
PUBLIC_SEMANTIC_FIELD_NAMES_FINAL = false
REAL_PS5_EXECUTION = false
'@

$ContractPath = Join-Path $Output 'STAGE83_SEMANTIC_CONTRACT.txt'
Write-Utf8NoBom -Path $ContractPath -Content $Contract

Write-Step 'Comprobar artefactos finales'
Require-File (Join-Path $Output 'stage83_verify.py') 'stage83_verify.py'
Require-File (Join-Path $Output 'STAGE83_REPORT.json') 'STAGE83_REPORT.json'
Require-File $ContractPath 'STAGE83_SEMANTIC_CONTRACT.txt'

$Report = Get-Content -LiteralPath (Join-Path $Output 'STAGE83_REPORT.json') -Raw -Encoding UTF8 | ConvertFrom-Json
if ($Report.overall -ne $true) {
    throw '[FAIL] STAGE83_REPORT.json indica overall != true.'
}
if ($Report.conclusions.FIELD_00_IS_DCB_GPU_ADDR -ne $true) {
    throw '[FAIL] field_00 no quedó demostrado.'
}
if ($Report.conclusions.FIELD_08_IS_DCB_NUM_DWORDS -ne $true) {
    throw '[FAIL] field_08 no quedó demostrado.'
}
if ($Report.conclusions.FIELD_0C_PADDING_CONFIRMED -ne $false) {
    throw '[FAIL] field_0c se cerró indebidamente.'
}

Write-Section 'Stage 83 completado'
Write-Host "  Python verifier  = $(Join-Path $Output 'stage83_verify.py')"
Write-Host "  Report            = $(Join-Path $Output 'STAGE83_REPORT.json')"
Write-Host "  Contract          = $ContractPath"
Write-Host '  FIELD_00_IS_DCB_GPU_ADDR    = True'
Write-Host '  FIELD_08_IS_DCB_NUM_DWORDS   = True'
Write-Host '  FIELD_0C_PADDING_CONFIRMED   = False'
