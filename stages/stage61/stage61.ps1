...
# ============================================================
# AGC PS5 Stage 61
# Dispatch Index (+0xA4) Value Provenance Audit
# ============================================================
...
$PreviousResults = 'D:\agc_work\stage60_results'
$OutputDir = 'D:\agc_work\stage61_results'

$Sdk = '/opt/ps5-payload-sdk'

$SprxWsl = '/mnt/d/agc_work/sce_stubs/libSceAgcDriver.sprx'
$NidDbWsl = '/mnt/d/sdk-master/sce_stubs/aerolib.csv'
$PreviousWsl = '/mnt/d/agc_work/stage60_results'
$OutputWsl = '/mnt/d/agc_work/stage61_results'

$WorkWsl = '/tmp/agc_stage61'

$AnalyzerWindows = Join-Path $OutputDir 'analyze_index_provenance.py'
$AnalyzerWsl = "$OutputWsl/analyze_index_provenance.py"

$StaticWindows = Join-Path $OutputDir 'stage61_static.json'
$SummaryWindows = Join-Path $OutputDir 'index_provenance_summary.txt'
$DisassemblyWindows = Join-Path $OutputDir 'index_provenance_disassembly.txt'
$CandidatesWindows = Join-Path $OutputDir 'index_write_candidates.txt'
$ReportWindows = Join-Path $OutputDir 'STAGE61_REPORT.json'
...
if (-not (Test-Path -LiteralPath $PreviousResults -PathType Container)) {
    throw "No existe Stage 60 results: $PreviousResults"
}

$PreviousStaticWindows = Join-Path $PreviousResults 'stage60_static.json'

if (-not (Test-Path -LiteralPath $PreviousStaticWindows -PathType Leaf)) {
    throw "No existe stage60_static.json: $PreviousStaticWindows"
}
...
cp $(Quote-Bash $AnalyzerWsl) \
   $(Quote-Bash "$WorkWsl/analyze_index_provenance.py")
...
Write-Step `
    'Rastrear procedencia del valor escrito en global_context + 0xA4'

$Analyze = @"
set -e

python3 $(Quote-Bash "$WorkWsl/analyze_index_provenance.py") \
    $(Quote-Bash $SprxWsl) \
    $(Quote-Bash $NidDbWsl) \
...
Write-Step 'Verificar artefactos Stage 61'

$VerifyArtifacts = @"
set -e

test -f $(Quote-Bash "$OutputWsl/stage61_static.json")
test -f $(Quote-Bash "$OutputWsl/index_provenance_summary.txt")
test -f $(Quote-Bash "$OutputWsl/index_provenance_disassembly.txt")
test -f $(Quote-Bash "$OutputWsl/index_write_candidates.txt")

echo '--- index_provenance_summary.txt ---'

cat $(Quote-Bash "$OutputWsl/index_provenance_summary.txt")
...
$IndexWriteProven = [bool]$Static.conclusions.DISPATCH_INDEX_WRITE_PROVEN
$IndexReadProven = [bool]$Static.conclusions.DISPATCH_INDEX_READ_PROVEN
$SpecialStoreFound = [bool]$Static.conclusions.SPECIAL_16_BYTE_INDEX_STORE_FOUND
$XmmOriginResolved = [bool]$Static.conclusions.XMM0_ORIGIN_PARTIALLY_RESOLVED
$ZeroCandidate = [bool]$Static.conclusions.ZERO_INITIALIZATION_CANDIDATE
$IndexSemanticsProven = [bool]$Static.conclusions.INDEX_SEMANTICS_PROVEN
$CountSemanticsProven = [bool]$Static.conclusions.COUNT_SEMANTICS_PROVEN
...
Write-Host "[INFO] stage61_static.json SHA256=$HashStatic"
Write-Host "[INFO] index_provenance_summary.txt SHA256=$HashSummary"
Write-Host "[INFO] index_provenance_disassembly.txt SHA256=$HashDisassembly"
Write-Host "[INFO] index_write_candidates.txt SHA256=$HashCandidates"
...
$Report = [ordered]@{
    ...
    conclusions = [ordered]@{
        DISPATCH_INDEX_WRITE_PROVEN = $IndexWriteProven
        DISPATCH_INDEX_READ_PROVEN = $IndexReadProven
        SPECIAL_16_BYTE_INDEX_STORE_FOUND = $SpecialStoreFound
        XMM0_ORIGIN_PARTIALLY_RESOLVED = $XmmOriginResolved
        ZERO_INITIALIZATION_CANDIDATE = $ZeroCandidate
        INDEX_SEMANTICS_PROVEN = $IndexSemanticsProven
        COUNT_SEMANTICS_PROVEN = $CountSemanticsProven
    ...
    hashes = [ordered]@{
        'stage61_static.json' = $HashStatic
        'index_provenance_summary.txt' = $HashSummary
        'index_provenance_disassembly.txt' = $HashDisassembly
        'index_write_candidates.txt' = $HashCandidates
    }
}
...
Write-Host 'Stage 61 completado' -ForegroundColor Green
Write-Host '============================================' -ForegroundColor Green
Write-Host ''

Write-Host "DISPATCH_INDEX_WRITE_PROVEN = $IndexWriteProven"
Write-Host "DISPATCH_INDEX_READ_PROVEN = $IndexReadProven"
Write-Host "SPECIAL_16_BYTE_INDEX_STORE_FOUND = $SpecialStoreFound"
Write-Host "XMM0_ORIGIN_PARTIALLY_RESOLVED = $XmmOriginResolved"
Write-Host "ZERO_INITIALIZATION_CANDIDATE = $ZeroCandidate"
Write-Host "INDEX_SEMANTICS_PROVEN = $IndexSemanticsProven"
Write-Host "COUNT_SEMANTICS_PROVEN = $CountSemanticsProven"
