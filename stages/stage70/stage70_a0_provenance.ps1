...
# ============================================================
# AGC PS5 Stage 70
# Global +0xA0 Value Provenance / Initialization Audit
# ============================================================
...
$PreviousStageDir = 'D:\agc_work\stage69_results'
$OutputDir = 'D:\agc_work\stage70_results'

$SprxWsl = '/mnt/d/agc_work/sce_stubs/libSceAgcDriver.sprx'
$NidDbWsl = '/mnt/d/sdk-master/sce_stubs/aerolib.csv'
$PreviousWsl = '/mnt/d/agc_work/stage69_results'
$OutputWsl = '/mnt/d/agc_work/stage70_results'

$TmpWsl = '/tmp/agc_stage70'
...
Write-Host ''
Write-Host '============================================'
Write-Host 'AGC PS5 Stage 70 - Global +0xA0 Value Provenance / Initialization Audit'
Write-Host '============================================'
...
if (-not (Test-Path -LiteralPath $PreviousStageDir)) {
    throw "No existe el directorio Stage 69: $PreviousStageDir"
}
...
$AnalyzerPath = Join-Path $OutputDir 'analyze_a0_provenance.py'
...
# ============================================================
# Copiar + validar analyzer
# ============================================================

$AnalyzerWsl = '/mnt/d/agc_work/stage70_results/analyze_a0_provenance.py'

$PrepareAnalyzerCommand = @"
cp '$AnalyzerWsl' '$TmpWsl/analyze_a0_provenance.py'

sed -i 's/\r$//' \
    '$TmpWsl/analyze_a0_provenance.py'
...
Invoke-WslStep `
    -Label 'Validar analizador Stage 70' `
...
$RunCommand = @"
python3 '$TmpWsl/analyze_a0_provenance.py' \
    '$SprxWsl' \
    '$NidDbWsl' \
    '$PreviousWsl' \
    '$OutputWsl'
...
Invoke-WslStep `
    -Label 'Rastrear procedencia real de global_context +0xA0' `
...
test -f '$OutputWsl/stage70_static.json'
test -f '$OutputWsl/a0_provenance_summary.txt'
test -f '$OutputWsl/a0_provenance_disassembly.txt'
test -f '$OutputWsl/a0_provenance.json'
test -f '$OutputWsl/STAGE70_REPORT.json'

echo '--- a0_provenance_summary.txt ---'
cat '$OutputWsl/a0_provenance_summary.txt'
...
Invoke-WslStep `
    -Label 'Verificar artefactos Stage 70' `
...
Invoke-WslStep `
    -Label 'Limpiar temporales Stage 70' `
...
$ReportPath = Join-Path $OutputDir 'STAGE70_REPORT.json'

if (-not (Test-Path -LiteralPath $ReportPath)) {
    throw "No existe STAGE70_REPORT.json"
}
...
Write-Host ''
Write-Host '============================================'
Write-Host 'Stage 70 completado'
Write-Host '============================================'
