...
# ============================================================
# AGC PS5 Stage 60
# Dispatch Table Initialization / Index Provenance Audit
# ============================================================
...
$PreviousResults = 'D:\agc_work\stage59_results'
$OutputDir = 'D:\agc_work\stage60_results'

$Sdk = '/opt/ps5-payload-sdk'

$SprxWsl = '/mnt/d/agc_work/sce_stubs/libSceAgcDriver.sprx'
$NidDbWsl = '/mnt/d/sdk-master/sce_stubs/aerolib.csv'
$PreviousWsl = '/mnt/d/agc_work/stage59_results'
$OutputWsl = '/mnt/d/agc_work/stage60_results'

$WorkWsl = '/tmp/agc_stage60'

$AnalyzerWindows = Join-Path $OutputDir 'analyze_dispatch_provenance.py'
$AnalyzerWsl = "$OutputWsl/analyze_dispatch_provenance.py"

$StaticWindows = Join-Path $OutputDir 'stage60_static.json'
$SummaryWindows = Join-Path $OutputDir 'dispatch_provenance_summary.txt'
$DisassemblyWindows = Join-Path $OutputDir 'dispatch_provenance_disassembly.txt'
$WritesWindows = Join-Path $OutputDir 'dispatch_write_sites.txt'
$IndexWritesWindows = Join-Path $OutputDir 'index_write_sites.txt'
$ReportWindows = Join-Path $OutputDir 'STAGE60_REPORT.json'
...
if (-not (Test-Path -LiteralPath $PreviousResults -PathType Container)) {
    throw "No existe Stage 59 results: $PreviousResults"
}

$PreviousStaticWindows = Join-Path $PreviousResults 'stage59_static.json'

if (-not (Test-Path -LiteralPath $PreviousStaticWindows -PathType Leaf)) {
    throw "No existe stage59_static.json: $PreviousStaticWindows"
}
...
cp $(Quote-Bash $AnalyzerWsl) \
   $(Quote-Bash "$WorkWsl/analyze_dispatch_provenance.py")
...
Write-Step `
    'Rastrear procedencia del índice +0xA4 y escrituras de la tabla dispatch'

$Analyze = @"
set -e

python3 $(Quote-Bash "$WorkWsl/analyze_dispatch_provenance.py") \
    $(Quote-Bash $SprxWsl) \
    $(Quote-Bash $NidDbWsl) \
...
Write-Step 'Verificar artefactos Stage 60'

$VerifyArtifacts = @"
set -e

test -f $(Quote-Bash "$OutputWsl/stage60_static.json")
test -f $(Quote-Bash "$OutputWsl/dispatch_provenance_summary.txt")
test -f $(Quote-Bash "$OutputWsl/dispatch_provenance_disassembly.txt")
test -f $(Quote-Bash "$OutputWsl/dispatch_write_sites.txt")
test -f $(Quote-Bash "$OutputWsl/index_write_sites.txt")

echo '--- dispatch_provenance_summary.txt ---'
cat $(Quote-Bash "$OutputWsl/dispatch_provenance_summary.txt")
...
$IndexProven = [bool]$Static.conclusions.DISPATCH_INDEX_FIELD_ACCESS_PROVEN
$LiteralWriteFound = [bool]$Static.conclusions.DISPATCH_TABLE_LITERAL_WRITE_FOUND
$PointerStoreWindowFound = [bool]$Static.conclusions.DISPATCH_POINTER_STORE_WINDOW_FOUND
$TargetDispatch = [bool]$Static.conclusions.TARGET_FUNCTION_CONTAINS_DISPATCH
$RuntimeResolved = [bool]$Static.conclusions.RUNTIME_FUNCTION_POINTER_RESOLVED
...
$HashWrites = Get-Sha256 $WritesWindows
$HashIndexWrites = Get-Sha256 $IndexWritesWindows

Write-Host "[INFO] stage60_static.json SHA256=$HashStatic"
Write-Host "[INFO] dispatch_provenance_summary.txt SHA256=$HashSummary"
Write-Host "[INFO] dispatch_provenance_disassembly.txt SHA256=$HashDisassembly"
Write-Host "[INFO] dispatch_write_sites.txt SHA256=$HashWrites"
Write-Host "[INFO] index_write_sites.txt SHA256=$HashIndexWrites"
...
$Report = [ordered]@{
    ...
    conclusions = [ordered]@{
        STAGE59_DISPATCH_PATTERN_CONFIRMED = $true
        DISPATCH_INDEX_FIELD_ACCESS_PROVEN = $IndexProven
        DISPATCH_TABLE_LITERAL_WRITE_FOUND = $LiteralWriteFound
        DISPATCH_POINTER_STORE_WINDOW_FOUND = $PointerStoreWindowFound
        TARGET_FUNCTION_CONTAINS_DISPATCH = $TargetDispatch
    ...
    hashes = [ordered]@{
        'stage60_static.json' = $HashStatic
        'dispatch_provenance_summary.txt' = $HashSummary
        'dispatch_provenance_disassembly.txt' = $HashDisassembly
        'dispatch_write_sites.txt' = $HashWrites
        'index_write_sites.txt' = $HashIndexWrites
    }
}
...
Write-Host 'Stage 60 completado' -ForegroundColor Green
Write-Host '============================================' -ForegroundColor Green
Write-Host ''

Write-Host 'STAGE59_DISPATCH_PATTERN_CONFIRMED = True'
Write-Host "DISPATCH_INDEX_FIELD_ACCESS_PROVEN = $IndexProven"
Write-Host "DISPATCH_TABLE_LITERAL_WRITE_FOUND = $LiteralWriteFound"
Write-Host "DISPATCH_POINTER_STORE_WINDOW_FOUND = $PointerStoreWindowFound"
Write-Host "TARGET_FUNCTION_CONTAINS_DISPATCH = $TargetDispatch"
Write-Host "RUNTIME_FUNCTION_POINTER_RESOLVED = $RuntimeResolved"
Write-Host 'BACKEND_CONSUMER_IDENTIFIED = False'
Write-Host 'FIELD_00_POINTER_SEMANTICS_PROVEN = False'
Write-Host 'FIELD_08_SIZE_SEMANTICS_PROVEN = False'
Write-Host 'FIELD_08_COUNT_SEMANTICS_PROVEN = False'
Write-Host 'FIELD_08_INDEX_SEMANTICS_PROVEN = False'
Write-Host 'FIELD_0C_FLAG_SEMANTICS_PROVEN = False'
Write-Host 'EXACT_STRUCT_SIZE_PROVEN = False'
