...
$PrevWin    = 'D:\agc_work\stage76_results'
$OutputWin  = 'D:\agc_work\stage77_results'
$SdkRoot    = '/opt/ps5-payload-sdk'

$SprxWsl    = '/mnt/d/agc_work/sce_stubs/libSceAgcDriver.sprx'
$NidDbWsl   = '/mnt/d/sdk-master/sce_stubs/aerolib.csv'
$PrevWsl    = '/mnt/d/agc_work/stage76_results'
$OutputWsl  = '/mnt/d/agc_work/stage77_results'
$StageWsl   = '/mnt/d/agc_ps5_stage77'

$AnalyzerName = 'analyze_helper_return_provenance.py'
$AnalyzerPath = Join-Path $OutputWin $AnalyzerName
$AnalyzerFinal = Join-Path $OutputWin $AnalyzerName

$Title = @'
============================================
AGC PS5 Stage 77 - Helper Return / Dispatch Table Argument Output Provenance Audit
============================================
'@
...
function Invoke-WSLScript {
    ...
    $stdout = & wsl.exe bash -lc $Script 2>&1
    $code = $LASTEXITCODE
...
Write-Utf8NoBom -Path $AnalyzerPath -Text $py
...
$prep = @"
set -e
rm -rf '$('/tmp/agc_stage77')'
mkdir -p '$('/tmp/agc_stage77')'
mkdir -p '$OutputWsl'
test -f '$SprxWsl'
test -f '$NidDbWsl'
test -d '$PrevWsl'
test -f '$((Convert-ToWslPath $AnalyzerPath))'
cp '$((Convert-ToWslPath $AnalyzerPath))' '/tmp/agc_stage77/$AnalyzerName'
sed -i 's/\r$//' '/tmp/agc_stage77/$AnalyzerName'
python3 -m py_compile '/tmp/agc_stage77/$AnalyzerName'
ls -lh '/tmp/agc_stage77/$AnalyzerName'
"@
Invoke-WSLScript $prep | Out-Null
Write-Host ""
...
$verify = @"
set -e
test -x '$SdkRoot/bin/prospero-clang'
test -x '$SdkRoot/bin/prospero-nm'
test -x '$SdkRoot/bin/prospero-lld'
python3 -c "from elftools.elf.elffile import ELFFile; print('pyelftools=OK')"
command -v objdump
command -v llvm-objdump
"@
Invoke-WSLScript $verify | Out-Null
...
Write-Host ""
Write-Host "==> Seguir caminos de retorno del helper 0x7CC0"
$run = @"
set -e
python3 '/tmp/agc_stage77/$AnalyzerName' \
    '$SprxWsl' \
    '$NidDbWsl' \
    '$PrevWsl' \
    '$OutputWsl'
"@
Invoke-WSLScript $run | Out-Null
...
Write-Host ""
Write-Host "==> Verificar artefactos Stage 77"
$check = @"
set -e
test -f '$OutputWsl/stage77_static.json'
test -f '$OutputWsl/dispatch_table_arg_summary.txt'
test -f '$OutputWsl/dispatch_table_arg_disassembly.txt'
test -f '$OutputWsl/STAGE77_REPORT.json'
echo '--- dispatch_table_arg_summary.txt ---'
cat '$OutputWsl/dispatch_table_arg_summary.txt'
echo '--- output files ---'
find '$OutputWsl' -maxdepth 1 -type f -print | sort
"@
Invoke-WSLScript $check | Out-Null
...
set -e
sha256sum \
    '$OutputWsl/stage77_static.json' \
    '$OutputWsl/dispatch_table_arg_summary.txt' \
    '$OutputWsl/dispatch_table_arg_disassembly.txt' \
    '$OutputWsl/STAGE77_REPORT.json'
"@
Invoke-WSLScript $hash | Out-Null

# ---------------------------------------------------------------------------
# Final
# ---------------------------------------------------------------------------

$reportWin = Join-Path $OutputWin 'STAGE77_REPORT.json'
if (-not (Test-Path -LiteralPath $reportWin)) {
    throw "No se generó $reportWin"
}

Write-Host ""
Write-Host "============================================"
Write-Host "Stage 77 completado"
Write-Host "============================================"

$report = Get-Content -Raw -LiteralPath $reportWin | ConvertFrom-Json
$con = $report.conclusions

Write-Host "HELPER_FUNCTION_DISASSEMBLED = $($con.HELPER_FUNCTION_DISASSEMBLED)"
Write-Host "HELPER_RETURN_PATHS_ENUMERATED = $($con.HELPER_RETURN_PATHS_ENUMERATED)"
Write-Host "HELPER_RETURNS_RSI_VALUE_PROVEN = $($con.HELPER_RETURNS_RSI_VALUE_PROVEN)"
Write-Host "HELPER_RETURNS_RDX_VALUE_PROVEN = $($con.HELPER_RETURNS_RDX_VALUE_PROVEN)"
Write-Host "MULTI_TABLE_ARGUMENT_RSI_PROVEN = $($con.MULTI_TABLE_ARGUMENT_RSI_PROVEN)"
Write-Host "SUBMIT_TABLE_ARGUMENT_RDX_PROVEN = $($con.SUBMIT_TABLE_ARGUMENT_RDX_PROVEN)"
Write-Host "TABLE_ARGUMENTS_FULLY_PROVEN = $($con.TABLE_ARGUMENTS_FULLY_PROVEN)"
Write-Host "TABLE_BASE_POINTER_MODEL_SUPPORTED = $($con.TABLE_BASE_POINTER_MODEL_SUPPORTED)"
Write-Host "DISPATCH_0x50_USE_CONFIRMED = $($con.DISPATCH_0x50_USE_CONFIRMED)"
Write-Host "DISPATCH_0x58_USE_CONFIRMED = $($con.DISPATCH_0x58_USE_CONFIRMED)"
Write-Host "ENTRY_STRIDE_PROVEN = $($con.ENTRY_STRIDE_PROVEN)"
Write-Host "INDEX_SEMANTICS_PROVEN = $($con.INDEX_SEMANTICS_PROVEN)"
Write-Host "COUNT_SEMANTICS_PROVEN = $($con.COUNT_SEMANTICS_PROVEN)"
Write-Host "FUNCTION_POINTER_VALUE_ORIGIN_PROVEN = $($con.FUNCTION_POINTER_VALUE_ORIGIN_PROVEN)"
Write-Host "EXACT_ENTRY_FIELD_NAMES_PROVEN = $($con.EXACT_ENTRY_FIELD_NAMES_PROVEN)"
Write-Host "EXACT_ENTRY_STRUCT_SIZE_PROVEN = $($con.EXACT_ENTRY_STRUCT_SIZE_PROVEN)"
Write-Host "BACKEND_CONSUMER_IDENTIFIED = $($con.BACKEND_CONSUMER_IDENTIFIED)"
Write-Host "SEMANTIC_PROTOTYPE_INFERRED = $($con.SEMANTIC_PROTOTYPE_INFERRED)"
Write-Host "EXECUTED_AGC = $($con.EXECUTED_AGC)"
Write-Host ""
