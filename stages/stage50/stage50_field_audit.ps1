...
param(
    ...
    [string]$OutDir = 'D:\agc_work\stage50_results',
...
$WorkWsl = '/tmp/agc_stage50'

$AnalyzerWin = Join-Path `
    $OutDir `
    'analyze_field_accesses.py'

$AnalyzerWsl = "$OutWsl/analyze_field_accesses.py"

$StaticWsl = "$WorkWsl/stage50_static.json"
$SummaryWsl = "$WorkWsl/field_summary.txt"
$DisassemblyWsl = "$WorkWsl/submit_family_disassembly.txt"
...
cp $(Quote-Bash $AnalyzerWsl) \
   $(Quote-Bash "$WorkWsl/analyze_field_accesses.py")
...
Write-Step 'Analizar LEA de contexto y accesos de memoria'

$Analyze = @"
set -e

python3 $(Quote-Bash "$WorkWsl/analyze_field_accesses.py") \
    $(Quote-Bash $SprxWsl) \
    $(Quote-Bash $NidDbWsl) \
    $(Quote-Bash $WorkWsl)
"@
...
# ============================================================
# Collect
# IMPORTANT: all Linux destinations are WSL paths.
# ============================================================
...
cp $(Quote-Bash $StaticWsl) \
   $(Quote-Bash "$OutWsl/stage50_static.json")

cp $(Quote-Bash $SummaryWsl) \
   $(Quote-Bash "$OutWsl/field_summary.txt")

cp $(Quote-Bash $DisassemblyWsl) \
   $(Quote-Bash "$OutWsl/submit_family_disassembly.txt")
...
# ============================================================
# Load generated report from Windows filesystem
# ============================================================

if (-not (Test-Path -LiteralPath $StaticWin -PathType Leaf)) {

    throw `
        "No se generó el archivo: $StaticWin"
}
...
$ContextAccesses = [int]$Static.context_memory_access_count

$ContextTracking = [bool]$Static.conclusions.CONTEXT_REGISTER_TRACKING
$ContextMemory = [bool]$Static.conclusions.CONTEXT_MEMORY_ACCESS_FOUND
$SubmitContext = [bool]$Static.conclusions.SUBMIT_CONTEXT_ACCESS_FOUND
$WriteSites = [bool]$Static.conclusions.WRITE_SITES_PROVEN
...
$HashDisasm = Get-Sha256 $DisassemblyWin

Write-Host `
    "[INFO] stage50_static.json SHA256=$HashStatic"

Write-Host `
    "[INFO] field_summary.txt SHA256=$HashSummary"

Write-Host `
    "[INFO] submit_family_disassembly.txt SHA256=$HashDisasm"
...
$Report = [ordered]@{
    ...
    counts = [ordered]@{
        ...
        context_memory_accesses = $ContextAccesses
    }

    conclusions = [ordered]@{
        CONTEXT_REGISTER_TRACKING = $ContextTracking
        CONTEXT_MEMORY_ACCESS_FOUND = $ContextMemory
        SUBMIT_CONTEXT_ACCESS_FOUND = $SubmitContext
        WRITE_SITES_PROVEN = $WriteSites
        ABI_PROTOTYPE_INFERRED = $false
        EXECUTED_AGC = $false
    }

    hashes = [ordered]@{
        'stage50_static.json' = $HashStatic
        'field_summary.txt' = $HashSummary
        'submit_family_disassembly.txt' = $HashDisasm
    }
}
...
Write-Host 'Stage 50 completado' -ForegroundColor Green
Write-Host '============================================' -ForegroundColor Green
Write-Host ''
